...
-- ------------------------------------------------------------------------------------------
-- Run this script to overwrite / install existing topics required by the skin
-- Set this to the StoreId that you are installing the skin on
DECLARE @StoreId INT = 1;

-- This will make sure we set the appropriate storeid on the topic table based on your current configuration
Declare @TopicStoreId INT = 0;
IF EXISTS(SELECT ConfigValue FROM GlobalConfig WHERE Name = 'AllowTopicFiltering' AND ConfigValue = 'true')
BEGIN 
	SET @TopicStoreId = @StoreId;
END

-- THIS WILL DELETE EXISTING TOPICS WITH THE SAME NAMES AS THE FOLLOWING --
delete from [dbo].[Topic] where Name = 'HomeTopIntro' AND StoreID = @TopicStoreId
	insert into [dbo].[Topic] ([TopicGUID], [Name], [Title], [HTMLOk], [Published], [IsFrequent], [StoreID], [Description]) 
	values (newid(), N'HomeTopIntro', N'Home Page Contents', 1, 1, 1, @TopicStoreId, N'
...
delete from [dbo].[Topic] where Name = 'HomePage.HomeImage' AND StoreID = @TopicStoreId
	insert into [dbo].[Topic] ([TopicGUID], [Name], [Title], [HTMLOk], [Published], [IsFrequent], [StoreID], [Description]) 
	values (newid(), N'HomePage.HomeImage', N'The main home page image area', 1, 1, 1, @TopicStoreId, N'
...
delete from [dbo].[Topic] where Name = 'Template.Logo' AND StoreID = @TopicStoreId
	insert into [dbo].[Topic] ([TopicGUID], [Name], [Title], [HTMLOk], [Published], [IsFrequent], [StoreID], [Description]) 
	values (newid(), N'Template.Logo', N'Template.Logo', 1, 1, 1, @TopicStoreId, N'
...
delete from [dbo].[Topic] where Name = 'Template.Footer' AND StoreID = @TopicStoreId
	insert into [dbo].[Topic] ([TopicGUID], [Name], [Title], [HTMLOk], [Published], [IsFrequent], [StoreID], [Description]) 
	values (newid(), N'Template.Footer', N'The footer section of the template', 1, 1, 1, @TopicStoreId, N'
	<div class="row">
